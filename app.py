    ...
    def King(self, x, y, grid):
        ...
        for move in self.vMoves:
            if 0 <= move[0] < 8 and 0 <= move[1] < 8:
                base = grid[move[0]][move[1]]
                sameColor = base != 0 and (self.white == base.isupper())
                if not sameColor:
                    self.vMoves2.append(move)

        return self.vMoves2

    def Queen(self, x, y, grid):
        return self.Rook(x, y, grid) + self.Bishop(x, y, grid)

    def Rook(self, x, y, grid):
        directions = [(1, 0), (-1, 0), (0, 1), (0, -1)]
        return self._generateSlidingMoves(x, y, grid, directions)
    ...
    def Knight(self, x, y, grid):
        ...
        for move in moves:
            ...
            if 0 <= newX < 8 and 0 <= newY < 8:
                base = grid[newX][newY]
                if base == 0 or (self.white != base.isupper()):
                    self.vMoves.append([newX, newY])
        return self.vMoves

    def Pawn(self, x, y, grid):
        direction = 1 if grid[x][y].islower() else -1
        startRow = 1 if grid[x][y].islower() else 6
        endRow = 7 if grid[x][y].islower() else 0

        # Move forward
        if grid[x + direction][y] == 0:
            self.vMoves.append([x + direction, y])
            if x == startRow and grid[x + 2 * direction][y] == 0:
                self.vMoves.append([x + 2 * direction, y])
        
        # Capture moves
        for dy in [-1, 1]:
            newY = y + dy
            if 0 <= newY < 8 and grid[x + direction][newY] != 0 and (self.white != grid[x + direction][newY].isupper()):
                self.vMoves.append([x + direction, newY])

        return self.vMoves

    def _generateSlidingMoves(self, x, y, grid, directions):
...
class Board:
    def __init__(self):
        self.sqs = [[pygame.Rect(j * 80, i * 80, 80, 80) for j in range(8)] for i in range(8)]
        self.colors = [[(200, 200, 200) if (i + j) % 2 == 0 else (30, 30, 30) for j in range(8)] for i in range(8)]
    ...
    def load_images(self):
        ...
        colors = ['White', 'Black']
        return [
            pygame.transform.scale(pygame.image.load(f"{color}{piece}.png"), (64, 85))
            for color in colors for piece in pieces
        ]
            ...
            for j in range(8):
                ...
                if piece:
                    ...
                    surface.blit(
                        image,
                        (
                            self.sqs[i][j].x + self.sqs[i][j].width / 2 - image.get_rect().width / 2,
                            self.sqs[i][j].y + self.sqs[i][j].height / 2 - image.get_rect().height / 2,
                        ),
                    )
                else:
                    pygame.draw.rect(surface, self.colors[i][j], self.sqs[i][j])
    ...
    def get_image(self, piece):
        color_index = 0 if piece.isupper() else 6
        piece_index = 'KQRBNP'.index(piece.upper())
        return self.IMAGES[color_index + piece_index]
    ...
    def colorValidMoves(self, x, y):
        ...
        for move in validMoves:
            self.colors[move[0]][move[1]] = (255, 121, 164)

    def resetColor(self):
        for i in range(8):
            for j in range(8):
                self.colors[i][j] = (200, 200, 200) if (i + j) % 2 == 0 else (30, 30, 30)
...
def notResultsInCheck(x1, y1, x2, y2):
    ...
    for i in range(8):
        for j in range(8):
            if white and grid2[i][j] == "K":
                kingX, kingY = i, j
            elif not white and grid2[i][j] == "k":
                kingX, kingY = i, j
            elif (grid2[i][j].isupper() != white):
                pos.append([i, j])

    for piece in pos:
        moves = validMoveGen.generateValidMoves(piece[0], piece[1], grid2)
        if [kingX, kingY] in moves:
            return False
    return True

def movePiece(x1, y1, x2, y2):
    global grid, turn
    if grid[x1][y1] != 0 and (turn % 2 == 1) == grid[x1][y1].isupper():
        if notResultsInCheck(x1, y1, x2, y2):
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            for i in range(8):
                for j in range(8):
                    if board.sqs[i][j].collidepoint(mousePos):
                        if not selected:
                            selectX, selectY = i, j
                            board.colorValidMoves(selectX, selectY)
                            selected = True
                        else:
                            moveX, moveY = i, j
                            movePiece(selectX, selectY, moveX, moveY)
                            board.resetColor()
                            selected = False
